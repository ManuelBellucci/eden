...
const Featured = () => {
  ...
  const fetchFeatured = async () => {
    try {
      const response = await axios.get('http://localhost:5000/listings?limit=5&page=1'); // Adjust this query as per your needs
      setFeatured(response.data.listings); // Make sure to access `.listings` here
      setLoading(false);
    } catch (error) {
  ...
  if (!Array.isArray(featured)) {
    return <div>Error: Expected an array of listings</div>;
  }
